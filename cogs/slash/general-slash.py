...
class General(commands.Cog, name="general-slash"):
    ...
    async def botinfo(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        """
        embed = disnake.Embed(
            description="Some useless info about the bot",
            color=0x9C84EF
        )
        embed.set_author(
        ...
        embed.add_field(
            name="Prefix:",
            value=f"/ (Slash Commands) only for now",
            inline=False
        )
        embed.set_footer(
            text="Used Krypton's template"
        )
    ...
    async def server(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        """
        embed = disnake.Embed(
            description=f"Join the support server for the bot by clicking [here](https://discord.gg/FnnMPKUh).",
            color=0xD75BF4
        )
        try:
            await interaction.author.send(embed=embed)
